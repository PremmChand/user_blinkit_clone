...
class UserViewModel(application:Application) : AndroidViewModel(application){
    ...
    suspend fun sendNotification(
        ...
        Log.d("Push Notification", "Fetching token for admin UID: $adminUid")

        val getTokenTask = FirebaseDatabase.getInstance()
            .getReference("Admins")
            .child("AdminInfo")
            .child(adminUid)
            .child("adminToken")
        ...
        getTokenTask.addOnCompleteListener { task ->
            if (!task.isSuccessful) {
                Log.e("Push Notification", "Error getting token: ${task.exception?.message}")
                return@addOnCompleteListener
            }

            val token = task.result.getValue(String::class.java)
            Log.d("Push Notification", "FB ReceivedToken: $token")
            ...
            functions
                ...
                .call(data) //.call(mapOf("data" to data))

                .addOnSuccessListener { result ->
                    Log.d("Push Notification", "Notification sent successfully: ${result.data}")
                }
                .addOnFailureListener { e ->
                    Log.e("Push Notification", "Failed to send notification: ${e.message}", e)
                }
        }
    }
